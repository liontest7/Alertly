...
function MiniSetting({ label, value, color = "text-white" }: { label: string, value: string, color?: string }) {
  return (
    <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 hover:border-zinc-700 transition-all group">
      <p className="text-[9px] text-white uppercase font-black mb-1 tracking-wider group-hover:text-zinc-200">{label}</p>
    ...
    <Card className="bg-zinc-950 border-zinc-900 p-6 rounded-[2rem] space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <Button 
            ...
            className="h-7 px-3 text-[10px] font-black text-white hover:text-[#5100fd] uppercase tracking-tighter bg-zinc-900/50 border border-zinc-800 rounded-lg"
          >
      ...
      <div className="pt-4 border-t border-zinc-900/50 space-y-4">
        <div className="flex items-center justify-between text-[10px] font-black text-white uppercase tracking-widest">
          <div className="flex items-center gap-2"><Target className="w-3 h-3 text-[#5100fd]" /> Monitors</div>
